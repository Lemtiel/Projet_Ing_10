...
with col_map:
    m_input = folium.Map(location=[20, 0], zoom_start=2)
    map_data = st_folium(m_input, height=400, width=700, key="input_map")

clicked_lat = map_data["last_clicked"]["lat"] if map_data and map_data.get("last_clicked") else 48.85
clicked_lon = map_data["last_clicked"]["lng"] if map_data and map_data.get("last_clicked") else 2.35

with col_params:
    col1, col2 = st.columns(2)
    with col1:
        st.markdown("**📍 Localisation**")
        lat       = st.number_input("Latitude",      value=float(round(clicked_lat, 4)))
        lon       = st.number_input("Longitude",     value=float(round(clicked_lon, 4)))
...
if st.session_state.prediction_done:
    ...
    lst_pred = st.session_state.lst_pred
    ...
    # Classification avec LST prédite
    input_clf = input_reg.copy()
    input_clf["Lst_celsius"] = lst_pred
    clf_features = [
        "Latitude", "Longitude", "Altitude_m", "Ndvi", "Albedo",
        "Cloud_cover_pct", "Lst_celsius", "day_sin", "day_cos"
    ]
    input_clf = input_clf[clf_features]
